...
def _aux_loss_score_backward_pytorch(
    intermediate_output: torch.Tensor,
    grad_scores: torch.Tensor,
    topk: int,
    score_function: str,
) -> torch.Tensor:
    """Compute aux-loss score backward using PyTorch-native ops.

    This replaces the fused CUDA backward kernel for the aux-loss score
    computation.  The backward is a pure element-wise (sigmoid) or
    reduction + element-wise (softmax) operation over the full [T, E]
    tensor — no topk or routing mask is involved.  PyTorch's optimised
    single-kernel primitives (``_softmax_backward_data``, element-wise
    fusions) significantly outperform the 1-warp-per-token custom CUDA
    kernel at large E.

    Parameters
    ----------
    intermediate_output : Tensor [T, E]
        Forward activation output (softmax probabilities or sigmoid values).
    grad_scores : Tensor [T, E]
        Upstream gradient w.r.t. the scores.
    topk : int
        Number of experts selected per token (affects sigmoid normalisation).
    score_function : str
        ``"softmax"`` or ``"sigmoid"``.

    Returns
    -------
    grad_logits : Tensor [T, E]
    """
    act = intermediate_output  # [T, E] — softmax probs or sigmoid values
    grad = grad_scores  # [T, E]

    if score_function == "softmax":
        # Use PyTorch's fused softmax backward kernel — single kernel launch,
        # computes grad_logits = act * (grad - dot(grad, act)) internally.
        # pylint: disable=protected-access
        grad_logits = torch._softmax_backward_data(grad, act, -1, act.dtype)
    elif score_function == "sigmoid":
        if topk > 1:
            # Undo normalisation backward (quotient rule):
            #   forward: scores[i] = σ[i] / (S + ε),  S = Σ σ[j]
            #   backward: dσ[i] = grad[i] / (S + ε) - dot(grad, σ) / (S + ε)²
            sum_act = act.sum(dim=-1, keepdim=True)  # [T, 1]
            inv_sum = 1.0 / (sum_act + 1e-20)
            dot_ga = (grad * act).sum(dim=-1, keepdim=True)  # [T, 1]
            d_act = grad * inv_sum - dot_ga * inv_sum * inv_sum
        else:
            d_act = grad
        # Sigmoid backward via PyTorch's fused kernel — single kernel launch,
        # computes d_act * σ * (1 - σ) internally.
        grad_logits = torch.ops.aten.sigmoid_backward(d_act, act)
    else:
...
class FusedComputeScoresForMoEAuxLoss(torch.autograd.Function):
    ...
    def backward(ctx, _, grad_scores):
        ...
        # Use PyTorch-native ops for the backward pass.  The aux loss backward
        # is a pure element-wise (sigmoid) or reduction+element-wise (softmax)
        # operation over the full [T, E] tensor — no topk/routing involved.
        # PyTorch's optimized kernels (vectorised loads, full-block parallelism)
        # significantly outperform the 1-warp-per-token fused CUDA kernel for
        # large E.
        grad_logits = _aux_loss_score_backward_pytorch(
            intermediate_output, grad_scores, ctx.topk, ctx.score_function,
        )
